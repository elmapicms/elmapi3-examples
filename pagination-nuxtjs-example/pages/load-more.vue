<template>
  <div class="container">
    ...
    <template v-else>
      ...
      <LoadMoreButton v-if="hasMore" :next-page="currentPage + 1" />
      <div v-else-if="allArticles.length > 0" class="loading" style="margin-top: 2rem">
  ...
  if (hasMore.value) {
    ...
    if (!loadedPages.value.has(nextPage)) {
      const { data: nextPageData } = await useAsyncData(
        `articles-load-more-check-next-${nextPage}`,
